 ...
 *   -"Ritira proposta": consente al configuratore di ritirare
 *     una proposta APERTA o CONFERMATA.
 *
...
public class ConfiguratoreView {
    ...
    // ================================================================
    /**
     * Mostra le proposte ritirabili (APERTE e CONFERMATE) e chiede quale
     * ritirare.
     *
     * Il ritiro e' una misura eccezionale (causa di forza maggiore). Viene
     * richiesta conferma esplicita prima di procedere.
     */
        ...
        while (true) {
            ...
            switch (scanner.nextLine().trim()) {
                case "a" -> {
                    ...
                    String err = controller.aggiungiCampoComune(n, chiediObbligatorio());
                    if (err.isEmpty()) {
                ...
                case "r" -> {
                    System.out.print("  Nome: ");
                    String n = scanner.nextLine().trim();
                    String err = controller.rimuoviCampoComune(n);
                    if (err.isEmpty()) {
                ...
                case "m" -> {
                    System.out.print("  Nome: ");
                    String n = scanner.nextLine().trim();
                    String err = controller.modificaObbligatorietaCampoComune(n, chiediObbligatorio());
                    if (err.isEmpty()) {
        ...
        while (true) {
            ...
            switch (scanner.nextLine().trim()) {
                ...
                case "m" -> {
                    System.out.print("  Nome: ");
                    String n = scanner.nextLine().trim();
                    String err = controller.modificaObbligatorietaCampoSpecifico(nomeCategoria, n, chiediObbligatorio());
                    if (err.isEmpty()) {
                        System.out.println("  Aggiornato.");
                    } else {
                        stampaErrore(err);
    ...
    private void visualizzaDettaglioCategoria() {
        ...
        if (controller.getCampiComuni().isEmpty()) {
            ...
            controller.getCampiComuni().forEach(c -> System.out.println("    - " + c.getNome() + "  [" + (c.isObbligatorio() ? "obbligatorio" : "facoltativo") + "]"));
        }
        System.out.println("\n  Campi SPECIFICI:");
        if (cat.getCampiSpecifici().isEmpty()) {
            System.out.println("    (nessuno)"); 
        }else {
            cat.getCampiSpecifici().forEach(c -> System.out.println("    - " + c.getNome() + "  [" + (c.isObbligatorio() ? "obbligatorio" : "facoltativo") + "]"));
        }
        premInvio();
    }
