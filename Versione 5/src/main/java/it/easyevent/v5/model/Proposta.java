...
public class Proposta {
    ...
    public String aggiungiAderente(String usernameF, LocalDate oggi) {
        if (usernameF == null || usernameF.isBlank()) return "Username non valido.";
        if (stato != StatoProposta.APERTA)            return "La proposta non e' in stato APERTA.";
        if (!isIscrizioneAperta(oggi))                return "Il termine ultimo di iscrizione e' scaduto.";
        if (isAderito(usernameF))                     return "Sei gia' iscritto a questa proposta.";
        int numMax = getNumeroMaxPartecipanti();
        if (numMax < 0) return "Numero di partecipanti non valido.";
        if (aderenti.size() >= numMax) return "La proposta ha raggiunto il numero massimo di partecipanti.";
    ...
    public String rimuoviAderente(String usernameF, LocalDate oggi) {
        if (stato != StatoProposta.APERTA)  return "Non e' possibile disdire un'iscrizione a una proposta non aperta.";
        if (!isIscrizioneAperta(oggi))      return "Il termine ultimo di iscrizione e' scaduto.";
        if (!isAderito(usernameF))          return "Non risulti iscritto a questa proposta.";
