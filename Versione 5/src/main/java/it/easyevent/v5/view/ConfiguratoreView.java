...
import it.easyevent.v5.controller.ConfiguratoreController;
 ...
 *   - Voce "9. Importa da file batch" (UC-CONF-09):
 ...
 *     La modalita' interattiva (voci 1-8) resta invariata.
 *
...
public class ConfiguratoreView {
     ...
     *   a) un singolo file (inserendo il percorso)
     *   b) più file in sequenza (inserendo i percorsi uno alla volta)
     *
    ...
    private void menuImportaBatch() {
        ...
        switch (scanner.nextLine().trim()) {
            case "s" -> importaSingoloFile();
            case "m" -> importaFilMultipli();
            case "e" -> mostraEsempioBatch();
            case "0" -> { /* torna */ }
            default  -> stampaErrore("Scelta non valida.");
        }
    }
    ...
    private void menuRitiraProposta() {
        ...
        catch (NumberFormatException e) { stampaErrore("ID non valido."); return; }
        Proposta scelta = ritirabili.stream().filter(p -> p.getId() == id).findFirst().orElse(null);
        if (scelta == null) { stampaErrore("Nessuna proposta ritirabile con ID " + id + "."); return; }
    ...
    private void menuCampiComuni() {
        while (true) {
            ...
            if (cc.isEmpty()) System.out.println("  (nessun campo comune)");
            else for (int i = 0; i < cc.size(); i++) System.out.printf("  %d. %s%n", i + 1, cc.get(i));
            System.out.println("\n  a. Aggiungi  r. Rimuovi  m. Modifica obbligatorieta'  0. Torna");
            System.out.print("  Scelta: ");
            switch (scanner.nextLine().trim()) {
                case "a" -> { System.out.print("  Nome: "); String n = scanner.nextLine().trim(); String e = controller.aggiungiCampoComune(n, chiediObbligatorio()); if (e.isEmpty()) System.out.println("  Campo '" + n + "' aggiunto."); else stampaErrore(e); }
                case "r" -> { System.out.print("  Nome: "); String n = scanner.nextLine().trim(); String e = controller.rimuoviCampoComune(n); if (e.isEmpty()) System.out.println("  Campo rimosso."); else stampaErrore(e); }
                case "m" -> { System.out.print("  Nome: "); String n = scanner.nextLine().trim(); String e = controller.modificaObbligatorietaCampoComune(n, chiediObbligatorio()); if (e.isEmpty()) System.out.println("  Aggiornato."); else stampaErrore(e); }
                case "0" -> { return; }
                default  -> stampaErrore("Scelta non valida.");
            }
        }
    ...
    private void menuCategorie() {
        while (true) {
            ...
            if (cats.isEmpty()) System.out.println("  (nessuna categoria)");
    ...
    private void menuCampiSpecifici() {
        ...
        while (true) {
            ...
            System.out.print("  Scelta: ");
            switch (scanner.nextLine().trim()) {
                case "a" -> { System.out.print("  Nome: "); String n = scanner.nextLine().trim(); if (n.isEmpty()) { stampaErrore("Nome vuoto."); break; } String e = controller.aggiungiCampoSpecifico(nomeCategoria, n, chiediObbligatorio()); if (e.isEmpty()) System.out.println("  Campo aggiunto."); else stampaErrore(e); }
                case "r" -> { System.out.print("  Nome: "); String n = scanner.nextLine().trim(); String e = controller.rimuoviCampoSpecifico(nomeCategoria, n); if (e.isEmpty()) System.out.println("  Campo rimosso."); else stampaErrore(e); }
                case "m" -> { System.out.print("  Nome: "); String n = scanner.nextLine().trim(); String e = controller.modificaObbligatorietaCampoSpecifico(nomeCategoria, n, chiediObbligatorio()); if (e.isEmpty()) System.out.println("  Aggiornato."); else stampaErrore(e); }
                case "0" -> { return; }
                default  -> stampaErrore("Scelta non valida.");
            }
        }
    ...
    private void nuovaProposta() {
        ...
        if (p == null) { stampaErrore("Categoria non trovata: " + nomeCat); return; }
    ...
    private void premInvio() {
        System.out.println(); System.out.print("  Premi INVIO per tornare..."); scanner.nextLine();
    }
